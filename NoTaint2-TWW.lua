...
if not NoTaint2_Proc_ResetActionButtonAction then
    ...
    local TXT_WARNING_CHAT_MESSAGE =
    "!!NoTaint2: Your action bars are tainted by [|cffffff00%s|r], reload UI to prevent further damage."
    ...
    function NoTaint2_ShowWarning(tainted_by)
        if GetTime() - last > 300 then
            ...
            if U1DBG then show_warn = not U1DBG.NT2S else show_warn = not notaint2twwstop end
            if show_warn then
...
if not NoTaint2_Proc_CleanActionButtonFlyout then
    ...
    ---------------------------------------------------------------]]
    local barsToUpdate = { MainMenuBar, MultiBarBottomLeft, MultiBarBottomRight, StanceBar, PetActionBar,
        PossessActionBar, MultiBarRight, MultiBarLeft, MultiBar5, MultiBar6, MultiBar7 }
